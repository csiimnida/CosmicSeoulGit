...
public class BloodWormDamageCheck : MonoBehaviour
{
    ...
    public void Attack1Checker(){
        Collider2D[] colliders =
            Physics2D.OverlapBoxAll(_parentObject.position, _bloodWorm.Attack1Size, _layerMask);
    ...
    public void Attack2Checker(){
        Collider2D[] colliders =
            Physics2D.OverlapBoxAll(_parentObject.position, _bloodWorm.Attack2Size, _layerMask);
        foreach (var collider in colliders)
        {
            Player player = collider.GetComponent<Player>();
            if(player != null && !player.ColCompo.isTrigger)
